...
exports.fetchAllResidents = fetchAllResidents;
exports.createResidentGenerator = createResidentGenerator;
...
function makeFiller(targetBytes) {
    let s = '';
    while (Buffer.byteLength(s) < targetBytes)
        s += FILLER_BASE + ' ';
...
async function fetchAllResidents(count) {
    ...
    for (let i = 0; i < count; i++)
        arr[i] = makeResident(i + 1);
    return arr;
}
async function* residentGenerator(count, opts = {}) {
